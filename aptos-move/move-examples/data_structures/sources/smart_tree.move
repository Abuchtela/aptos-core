...
module aptos_std::smart_tree {
    ...
    struct Node has drop, store {
        ...
        // The node index of its previous node at the same layer, or NULL_INDEX if it doesn't have a previous node.
        prev: u64,
        // The node index of its next node at the same layer, or NULL_INDEX if it doesn't have a previous node.
        next: u64,
    }

    /// The metadata of a child of a node.
    struct Child has copy, drop, store {
        // The max key of it's child, or the key if the current node is a leaf node.
        max_key: u64,
