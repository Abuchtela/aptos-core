...
pub(crate) struct LatestView<'a, T: Transaction, S: TStateView<T::Key>> {
    base_view: &'a S,
    latest_view: ViewMapKind<'a, T>,
    txn_idx: TxnIndex,
}

impl<'a, T: Transaction, S: TStateView<T::Key>> LatestView<'a, T, S> {
...
impl<'a, T: Transaction, S: TStateView<T::Key>> TStateView<T::Key> for LatestView<'a, T, S> {
